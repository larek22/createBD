...
- Лексический буст для описательных и keyword-запросов + гарантированный приоритет прямых номеров статей.
- Авто-рекомендации по результатам тестов: подсказки, что улучшить в корпусе и метаданных.
...
def extract_query_terms(text: str) -> List[str]:
    return re.findall(r"[А-Яа-яA-Za-z0-9]{3,}", text.lower())
...
def build_quality_advice(per_label: Dict[str, Dict[str, int]], failures: List[Dict[str, Any]]) -> str:
    advice_items: List[str] = []

    def rate(stats: Dict[str, int], key: str) -> float:
        total = max(1, stats.get("total", 0))
        return stats.get(key, 0) / total

    summary_stats = per_label.get("summary")
    if summary_stats and summary_stats.get("total", 0):
        top1_rate = rate(summary_stats, "top1")
        if top1_rate < 0.7:
            advice_items.append(
                "Улучшите обработку описательных запросов: добавьте более мелкие чанки или увеличьте вес текстового совпадения."
            )

    keyword_stats = per_label.get("keywords")
    if keyword_stats and keyword_stats.get("total", 0):
        top1_rate_kw = rate(keyword_stats, "top1")
        if top1_rate_kw < 0.7:
            advice_items.append(
                "Пересмотрите список ключевых слов: расширьте topics и semantic_summary синонимами по проблемным терминам."
            )

    article_stats = per_label.get("article")
    if article_stats and article_stats.get("total", 0) and rate(article_stats, "top1") < 0.95:
        advice_items.append("Проверьте полноту индекса статей: прямые номера должны гарантированно возвращаться первыми.")

    if failures:
        problematic_articles: Dict[str, int] = {}
        for row in failures:
            expected = row.get("expected")
            if expected:
                problematic_articles[expected] = problematic_articles.get(expected, 0) + 1
        top_problem = sorted(problematic_articles.items(), key=lambda x: x[1], reverse=True)[:5]
        if top_problem:
            formatted = ", ".join(f"ст. {safe_html(k)}" for k, _ in top_problem)
            advice_items.append(
                f"Перепроверьте исходные документы для {formatted}: проверьте наличие текста и корректность метаданных."
            )

    if not advice_items:
        advice_items.append(
            "Результаты стабильны: серьёзных провалов не обнаружено. Продолжайте отслеживать динамику качества."
        )

    return "<ul>" + "".join(f"<li>{item}</li>" for item in advice_items) + "</ul>"
...
class SearchEngine:
    ...
    def run(self, options: SearchOptions) -> SearchOutcome:
        ...
        candidate_ids = list(fused_all.keys())
        candidate_points = self._points_from_ids(candidate_ids)

        query_terms = extract_query_terms(qtxt)
        query_terms_set = set(query_terms)
        norm_query = normalize_for_similarity(qtxt)
        norm_query_tokens = set(norm_query.split())
        is_summary_like = len(norm_query_tokens) >= 12 or len(qtxt) >= 160 or qtxt.count("\n") >= 1
        is_keyword_like = (
            bool(query_terms_set)
            and len(query_terms_set) <= 5
            and len(norm_query_tokens) <= 12
            and not re.search(r"ст\.", qtxt, re.IGNORECASE)
        )

        lexical_stats = {"topic_boosted": 0, "summary_boosted": 0, "keyword_boosted": 0}
        ...
        for p in candidate_points:
            ...
            for t in payload.get("topics") or []:
                topic_tokens.update(extract_query_terms(str(t)))

            if query_terms_set and topic_tokens:
                overlap = len(topic_tokens & query_terms_set)
                if overlap:
                    boost = overlap * (0.28 if is_keyword_like else 0.14)
                    fused_all[pid] = fused_all.get(pid, 0.0) + boost
                    lexical_stats["topic_boosted"] += 1

            summary_blob = (payload.get("semantic_summary") or "")
            preview_words = " ".join((payload.get("text") or "").split()[:90])
            combined_preview = f"{summary_blob} {preview_words}".strip()
            combined_norm = normalize_for_similarity(combined_preview)

            if is_summary_like and norm_query and combined_norm:
                ratio = SequenceMatcher(None, norm_query[:800], combined_norm[:800]).ratio()
                if ratio > 0.2:
                    fused_all[pid] = fused_all.get(pid, 0.0) + ratio * 1.35
                    lexical_stats["summary_boosted"] += 1
            elif query_terms_set and combined_norm:
                candidate_tokens = set(combined_norm.split())
                overlap_tokens = len(candidate_tokens & query_terms_set)
                if overlap_tokens:
                    fused_all[pid] = fused_all.get(pid, 0.0) + min(overlap_tokens, 6) * 0.06
                    lexical_stats["keyword_boosted"] += 1
        ...
        priority_points = direct_points + ranked_points
        ranked_points = dedup_limit(priority_points, max(k * 2, k + 2))
        ...
        debug_meta.update(
            {
                ...
                "lexical_mode": "summary" if is_summary_like else ("keywords" if is_keyword_like else "hybrid"),
...
class QualityWorker(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            advice_html = build_quality_advice(per_label, failures)
            table_rows = [
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def export_quality_report(self):
        ...
        html = (
            ...
            + "<h3>Рекомендации</h3>"
            + self.quality_last_report.advice_html
            + "<h3>Детализация</h3>"
    ...
    def do_search(self):
        ...
        lexical_mode = debug_meta.get("lexical_mode")
        if lexical_mode:
            mode_map = {
                "summary": "Лексическое усиление: длинный описательный запрос",
                "keywords": "Лексическое усиление: короткий набор ключевых слов",
                "hybrid": "Лексическое усиление: комбинированный режим",
            }
            meta_lines.append(mode_map.get(lexical_mode, f"Лексический режим: {safe_html(str(lexical_mode))}"))
            stats = debug_meta.get("lexical_stats") or {}
            if isinstance(stats, dict) and any(stats.values()):
                meta_lines.append(
                    "Усиленные совпадения — темы: {topic_boosted}, описания: {summary_boosted}, ключевые слова: {keyword_boosted}".format(
                        topic_boosted=stats.get("topic_boosted", 0),
                        summary_boosted=stats.get("summary_boosted", 0),
                        keyword_boosted=stats.get("keyword_boosted", 0),
                    )
                )
